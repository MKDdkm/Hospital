...
const VitalBadge = ({ label, value, unit, icon }: { label: string; value: string; unit: string; icon: React.ReactNode }) => (
  <div className="flex flex-col items-center rounded-xl border border-slate-200 bg-white px-3 py-2 min-w-[72px]">
    <span className="text-[#2872a1] mb-0.5">{icon}</span>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
              className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs text-slate-700 focus:outline-none focus:border-[#2872a1]" />
            <span className="text-xs text-slate-400">–</span>
            <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)}
              className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs text-slate-700 focus:outline-none focus:border-[#2872a1]" />
            {(dateFrom || dateTo) && (
              <button onClick={() => { setDateFrom(""); setDateTo(""); }} className="text-xs text-[#2872a1] hover:underline">Clear</button>
            )}
          </div>
        </div>
                  ...
                  {latestVitals && (
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500 mb-2 inline-flex items-center gap-1.5">
                        <Activity className="h-3.5 w-3.5 text-[#2872a1]" /> Latest Vitals
                      ...
                      {latestVitals.notes && (
                        <p className="mt-2 text-xs text-slate-500 italic border-l-2 border-[#2872a1]/30 pl-2">{latestVitals.notes}</p>
                      )}
                    </div>
                  )}
                        ...
                        {patientAppts.map((a) => (
                          <div key={a.id} className="rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2.5">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              ...
                              <div className="flex gap-1.5">
                                <button
                                  ...
                                  className="inline-flex items-center gap-1 rounded-full border border-[#2872a1]/20 bg-[#2872a1]/5 px-2.5 py-1 text-[11px] font-semibold text-[#2872a1] hover:bg-[#2872a1]/10 transition-colors"
                                >
                            ...
                            {diagnosisNotes[a.id] && editingNote?.apptId !== a.id && (
                              <p className="mt-1.5 text-xs text-slate-600 italic border-l-2 border-[#2872a1]/30 pl-2">{diagnosisNotes[a.id]}</p>
                            )}
                            {editingNote?.apptId === a.id && (
                              <div className="mt-2 flex gap-2">
                                <input
                                  ...
                                  className="flex-1 rounded-lg border border-[#2872a1]/30 bg-white px-2.5 py-1.5 text-xs text-slate-700 focus:outline-none focus:border-[#2872a1]"
                                />
                      ...
                      <div className="space-y-1.5">
                        {patientRx.map((rx) => (
                          <div key={rx.id} className="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2 text-xs flex items-center justify-between">
                            <div>
                              <span className="font-semibold text-[#2872a1]">{rx.id}</span>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-white/60 bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-slate-800 inline-flex items-center gap-2">
                <Activity className="h-4 w-4 text-[#2872a1]" /> Record Vitals
              </h3>
                ...
                <div key={field} className="space-y-1">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="h-9 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 pr-10 text-sm font-semibold text-slate-800 focus:border-[#2872a1] focus:outline-none"
                    />
            ...
            <div className="mt-3 space-y-1">
              ...
              <input
                ...
                className="h-9 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 focus:border-[#2872a1] focus:outline-none"
              />
            </div>

            <div className="mt-4 flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setVitalsModal(null)}>Cancel</Button>
              <Button onClick={handleSaveVitals} className="bg-[#2872a1] hover:bg-[#1a4d73] inline-flex items-center gap-2">
