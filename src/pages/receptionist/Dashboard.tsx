...
import { Users, CalendarCheck, Receipt, Activity } from "lucide-react";

const stats = [
  { label: "Total Patients", value: patients.length, icon: <Users className="w-6 h-6" />, color: "text-primary" },
  { label: "Today's Appointments", value: appointments.filter(a => a.date === "2026-03-28").length, icon: <CalendarCheck className="w-6 h-6" />, color: "text-success" },
  { label: "Pending Bills", value: bills.filter(b => b.status === "Pending").length, icon: <Receipt className="w-6 h-6" />, color: "text-warning" },
  { label: "Completed Today", value: appointments.filter(a => a.status === "Completed").length, icon: <Activity className="w-6 h-6" />, color: "text-accent" },
];
  ...
  <DashboardLayout>
    <div className="mx-auto w-full max-w-[1400px] space-y-8 animate-fade-in-up">
      <section className="overflow-hidden rounded-3xl border border-blue-100 bg-gradient-to-r from-blue-50 via-cyan-50 to-sky-50 p-5 text-slate-900 shadow-[0_18px_45px_-34px_rgba(37,99,235,0.35)] sm:p-7">
        <h1 className="dashboard-title text-slate-900">Receptionist Dashboard</h1>
        <p className="mt-1 text-sm text-slate-600">Manage front-desk operations, appointments, and payment follow-ups.</p>
      </section>
            ...
            <tbody>
              {appointments.filter(a => a.date === "2026-03-28").map((a) => (
                <tr key={a.id} className="border-b border-blue-50 last:border-0 hover:bg-blue-50/40 transition-colors">
                  ...
                  <td className="px-4 sm:px-6 py-3 text-slate-700">{a.doctor}</td>
                  <td className="px-4 sm:px-6 py-3 text-slate-700">{a.time}</td>
                  <td className="px-4 sm:px-6 py-3">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                      a.status === "Completed" ? "bg-cyan-100 text-cyan-700" :
